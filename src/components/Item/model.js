import { makeAutoObservable } from "mobx";

export class ItemModel {
  formData = new FormData();
  formValues = {};
  lengthImages = 0;
  indexDeleteImages = [];
  image = [
    {
      data: "",
    },
  ];
  updating = false;
  constructor(props) {
    makeAutoObservable(this);
    this.formValues = props.props;
    this.listModel = props.listModel;
    if (props.image !== undefined) {
      props.image.data.map((img) => this.image.push({ data: img }));
    }
    this.lengthImages = this.image.length - 1;
  }

  async cleanUpdate(props) {
    if (window.confirm("Do you really want to leave updatepage?")) {
      this.listModel.onInit();
    }
  }

  onChange = (event) => {
    this.formValues[event.target.name] = event.target.value;
  };
  deleteChooseImage(index) {
    this.formValues.images[index] = null;
    console.log(this.formValues.images);
  }

  deleteImage(index, _id) {
    const formData = new FormData();
    this.indexDeleteImages.push(index + 1);
    let id = this.formValues.images[index];
    this.formValues.idDelete += "," + id._id.toString();
    console.log(id._id);
    console.log(this.formValues.idDelete);
    formData.append("idDelete", this.formValues.idDelete);
    formData.append("_id", this.formValues._id);
    fetch("/heros/update", {
      method: "POST",
      body: formData,
    })
      .then((response) => response.json())
      .then((result) => {
        console.log(result);
      })
      .catch((error) => {
        console.error("Error:", error);
      });
  }

  changeImage(event) {
    console.log(event.target.files.length);
    console.log(event.target.files);
    for (let i = 0; i < event.target.files.length; i++) {
      console.log(event.target.files[i]);
      this.formValues.images.push(event.target.files[i]);
    }
  }

  async updateHero() {
    console.log(this.formValues.idDelete);
    this.formValues.images.map(
      (img) => img._id === undefined && this.formData.append("images", img)
    );
    this.formValues.nickname &&
      this.formData.append("nickname", this.formValues.nickname);
    this.formValues.real_name &&
      this.formData.append("real_name", this.formValues.real_name);
    this.formValues.origin_description &&
      this.formData.append(
        "origin_description",
        this.formValues.origin_description
      );
    this.formValues.superpowers &&
      this.formData.append("superpowers", this.formValues.superpowers);
    this.formValues.catch_phrase &&
      this.formData.append("catch_phrase", this.formValues.catch_phrase);
    this.formData.append("_id", this.formValues._id);
    fetch("/heros/update", {
      method: "POST",
      body: this.formData,
    })
      .then((response) => response.json())
      .then((result) => {
        console.log(result);
      })
      .catch((error) => {
        console.error("Error:", error);
      });
  }

  deleteHero() {
    fetch("/heros/delete", {
      method: "POST",
      body: JSON.stringify({ id: this.formValues._id }),
      mode: "cors",
      headers: {
        "Content-Type": "application/json",
      },
    })
      .then((res) => res.json())
      .then((json) => {
        if (json) {
          console.log(json);
        }
      });
  }
}
